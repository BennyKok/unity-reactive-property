        ...
        public override void OnInspectorGUI()
        {
            ...
            if (sourceObject.objectReferenceValue && sourceObject.objectReferenceValue is GameObject)
            {
                ...
                if (source.objectReferenceValue)
                    selectedIndex = coms.ToList().FindIndex(x => x == source.objectReferenceValue);
                using (new EditorGUILayout.HorizontalScope())
                {
                    EditorGUILayout.PrefixLabel("Components");
                    EditorGUI.BeginChangeCheck();
                    var index = EditorGUILayout.Popup(selectedIndex,
                        coms.ToList().ConvertAll<string>(x => x.GetType().Name).ToArray());
